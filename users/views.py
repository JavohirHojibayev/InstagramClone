...
class ForgotPasswordView(APIView):
    ...
    def post(self, request, email_or_phone=None, *args, **kwargs):
        serializer = self.serializer_class(data=request.data)
        serializer.is_valid(raise_exception=True)
        email_or_username = serializer.validated_data.get("email_or_phone")
        user = serializer.validated_data.get("user")
        if check_email_or_phone(email_or_username) == "phone":
            code = user.create_verify_code(VIA_PHONE)
            send_email(email_or_phone, code)
        elif check_email_or_phone(email_or_phone) == "emial":
            code = user.create_verify_code(VIA_EMAIL)
            send_email(email_or_phone, code)
        ...
        return Response(
            {
                "success": True,
                "message": "tasdiqlash kodingiz yuborildi",
                "access":user.token()['access'],
                "refresh":user.token()['refresh_token'],
